...
Store durable loop state under the Codex home directory, not inside the project checkout:

```bash
CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"
STATE_ROOT="$CODEX_HOME/superpowers/state-index/<repo-id>"
```

```text
~/.codex/superpowers/state-index/<repo-id>/
  index.json
  entities/
...
Use a stable `<repo-id>` such as `mortgage` for `/Users/bhuang/workspace/mortgage`. Record `repo_root` in state so the global facts remain tied to the real checkout.
...
```json
{
  ...
  "repo_id": "mortgage",
  "repo_root": "/Users/bhuang/workspace/mortgage",
  "repo": "owner/repo",
  "pr_number": 123,
...
## Execution Context
Repo ID: mortgage
Repo root: /Users/bhuang/workspace/mortgage
...
```json
{
  ...
  "repo_id": "mortgage",
  "repo_root": "/Users/bhuang/workspace/mortgage",
  "main_repo_path": "/Users/bhuang/workspace/mortgage",
  "worktree_path": "/Users/bhuang/workspace/mortgage-wt-pr-123",
  "branch": "feature/foo",
...
2. Read the entity state and associated loop summaries from `STATE_ROOT`.
...
**Writing state into the repo checkout**

Problem: State dirties the project git status or disappears when a worktree is removed.

Fix: Write durable state under `~/.codex/superpowers/state-index/<repo-id>/`.
